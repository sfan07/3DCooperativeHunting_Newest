...
if __name__ == '__main__':
    ...
    # target crazyflie setup
    tf_num = 2
    params = cf_params['cf' + str(tf_num)]
    params.update(common_params)
    cf_target = Crazyflie(params)

    agent_state = np.zeros((num_agents, 9))
    # initialize target state, hardcode for now
    target_height = 1
    target_waypts = np.array([[ 1.5,  0, target_height, 0, 0],
                              [  -1,  1, target_height, 0, 0],
                              [  -1, -1, target_height, 0, 0],
                              [ 1.5,  0, target_height, 0, 0]])
    point_idx = 0

    # vertex distance
    l = 0.30

    # look_ahead params for interpolation
    look_ahead_num = 10
    dt = 0.2    
    ...
    c1_alp = 6    # float, gain for the alpha position term
    c2_alp = 2    # float, gain for the alpha velocity term
    d = 0.4       # float, optimal separation distance between agents for an agent
    d_p = 0.3     # float, distance where the repulsive potential goes to 0
    ...
    try:
        ...
        while True:
            ...
            agent_traj.append(agent_state.tolist())

            # collision avoidance
            acc = avoidance_controller.get_control_all(agent_state[:,:6])

            # get target stat 
            target_state = cf_target.get_state()[0]
            target_traj.append(target_state.tolist())

            # get unit agent and vertex vectors, centered at the target
            vertex_pos = get_vertices(target_state[:3], l)
            vertex_vec = normalize(vertex_pos-target_state[:3], axis = 1, norm = 'l2')
            agent_vec = normalize(agent_state[:,:3]-target_state[:3], axis = 1, norm = 'l2')

            # hungarian algorithm for assignment
            assignment = hungarian_assignment(vertex_vec, agent_vec)
            assigned_vertex_pos = vertex_pos[assignment]
            ...
            for j in range(num_agents):
                ...
                vx, vy = vx_interp(5*dt)+0.5*target_state[3], vy_interp(5*dt)+0.5*target_state[4]
                ax, ay = ax_interp(5*dt), ay_interp(5*dt)
                ...
                if active[j]:
                    ...
                    # check if cf has reached the assigned vertex
                    print(assignment)
                    if norm(agent_state[j,:3]-assigned_vertex_pos[j]) < 0.20 and norm(agent_state[j, 3:6]) < 0.15:
                        reached[j] = True
                    else:
                        reached[j] = False

            # send cmd_vel for target
            cf_target.approach(target_waypts[point_idx,:3], 0, 0, 0, 0)

            if np.all(reached == True) and time.time()-t > 1: # time.time() to prevent continous increment
                point_idx = (point_idx + 1) % target_waypts.shape[0]
                reached = np.array([False]*num_agents) | ~active
                t = time.time()
        ...
        while not np.all(reached) or not target_reached:
            for i,cf in enumerate(cfs):
                reached[i] = cf.land()
            target_reached = cf_target.land()
        ...
        np.save('/home/charles/catkin_ws/src/CoHunting3D_exp/trajectories/'+filename, agent_traj)
        filename = timestamp.strftime("target_traj%m_%d_%Y_%H_%M_%S")
        np.save('/home/charles/catkin_ws/src/CoHunting3D_exp/trajectories/'+filename, target_traj)
