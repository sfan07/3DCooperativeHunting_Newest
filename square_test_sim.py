...
class Square_Sim2D():   
    def __init__(self, agent_init, target_init, obs_init, num_iter, dt, order = 1):
        ...
        self.pids = [PID(kp, ki, kd, u_min, u_max, self.dt),\
       	...
       	self.points = np.array([[ 0.5,  0.5, 1],
		                       [-0.5,  0.5, 1], 
		                       [-0.5, -0.5, 1],
		                       [ 0.5, -0.5, 1]])
        self.point_idx = 0
...
​
    ...
    def run(self):
        ...
        for i in range(self.num_iter):
            ...
            pos = np.hstack((self.agent_state.T[0,:2], self.agent_h[0,0]))
            vel = np.hstack((self.agent_state.T[0,2:], self.agent_h[0,1]))
            # check for reach
            if norm(pos - waypt) < 0.05 and norm(vel) < 0.15:
            	self.point_idx = (self.point_idx + 1)%self.points.shape[0]
            	waypt = self.points[self.point_idx]

            ref_state[:2, 0] = waypt[:2]
            acc = self.pids[0].control(waypt[-1], self.agent_h[0, 0])
            self.agent_h[0, 0] += self.agent_h[0, 1]*self.dt+(1/2)*acc*self.dt*self.dt
            self.agent_h[0, 1] += acc
    ...
    def vis(self):
        ...
        ax.set_xlim3d(-2, 2, auto = False)
        ax.set_ylim3d(-3, 3, auto = False)  
...
if __name__ == '__main__':
    agent_init = np.zeros((6, 1)) 

    agent_init[:2, 0] =  -0.8, -0.3

    target_init = None
    obs_init = None

    square_Sim2D = Square_Sim2D(agent_init, target_init, obs_init, num_iter = 10000, dt = 0.01, order = 1)
    square_Sim2D.run()
